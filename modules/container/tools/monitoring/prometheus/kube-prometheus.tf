...
%{ endif ~}
  ...
  persistence:
    type: pvc
    enabled: true
    storageClassName: ${var.grafana_storage_sku == "Standard" ? "default-encrypted" : "managed-premium-encrypted"}
    accessModes: [ ReadWriteOnce ]
...
%{ endif ~}
    ...
    ##
    externalUrl: "${var.ingress_host_name}/prometheus"
...
%{ endif ~}
    ...
    ##
       volumeClaimTemplate:
         spec:
           storageClassName: ${var.prometheus_storage_sku == "Standard" ? "default-encrypted" : "managed-premium-encrypted"}
           accessModes: ["ReadWriteOnce"]
           resources:
             requests:
